...
const { colours, api_keys } = require("../config.json")
...
module.exports.run = async (client, message, args) => {
    ...
    if (langs[lang.toLowerCase()]) lang = lang.toLowerCase();
        ...
        if (x) {
            ...
            return message.channel.send(
                new RichEmbed()
                    .setDescription("Invalid language provided.")
    ...
    request({
        uri: `https://translate.yandex.net/api/v1.5/tr.json/detect?key=${api_keys.yandex}&text=${encodeURI(text)}`,
        json: true
    }).then(language => {
        request({
            uri: `https://translate.yandex.net/api/v1.5/tr.json/translate?key=${api_keys.yandex}&text=${encodeURI(text)}&lang=${language.lang}-${encodeURI(lang)}&format=html`,
            json: true
        }).then(data => {
